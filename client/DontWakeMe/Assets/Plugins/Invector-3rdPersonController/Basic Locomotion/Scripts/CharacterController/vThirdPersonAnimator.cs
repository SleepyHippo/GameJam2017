    ...
    public abstract class vThirdPersonAnimator : vThirdPersonMotor
    {
        ...
        void LocomotionAnimation()
        {
            ...
            if (isStrafing)
            {
                ...
                animator.SetFloat("InputHorizontal", lockMovement ? 0f : direction, 0.2f, Time.fixedDeltaTime);
                StrafeTurningAnimation();
            }                

            animator.SetFloat("InputVertical", !stopMove || lockMovement ? speed : 0f, 0.2f, Time.fixedDeltaTime);
        }

        protected void ExtraMoveSpeed()
        {
            if (stopMove || actions || lockMovement) return;
            var speedDir = new Vector2(direction, speed);
            var strafeSpeed = (isSprinting ? 1.5f : 1f) * Mathf.Clamp(speedDir.magnitude, 0f, 1f);
            // strafe extra speed
            if (baseLayerInfo.IsName("Strafing Movement") && isStrafing)
            {
                if (strafeSpeed <= 0.5f)
                    ExtraStrafeSpeed(strafeWalkSpeed * strafeSpeed);
                else if (strafeSpeed > 0.5f && strafeSpeed <= 1f)
                    ExtraStrafeSpeed(strafeRunningSpeed * strafeSpeed);
                else
                    ExtraStrafeSpeed(strafeRunningSpeed * strafeSpeed);
            }
            // crouch strafe extra speed
            else if (baseLayerInfo.IsName("Strafing Crouch"))
                ExtraStrafeSpeed(strafeCrouchSpeed * strafeSpeed);

            // free extra speed                
            else if (baseLayerInfo.IsName("Free Movement") || baseLayerInfo.IsName("JumpMove") && !isStrafing)
            {
                if (speed <= 0.5f)
                    ExtraFreeSpeed(freeWalkSpeed * speed);
                else if (speed > 0.5 && speed <= 1f)
                    ExtraFreeSpeed(freeRunningSpeed * speed);
                else
                    ExtraFreeSpeed(freeSprintSpeed * speed);
            }
            //crouch extra speed
            else if (baseLayerInfo.IsName("Free Crouch"))
                ExtraFreeSpeed(freeCrouchSpeed * speed);           
        }
       
        void StrafeTurningAnimation()
        {
            // used while strafing
	        if (isCrouching || lockMovement) return; 
            
            var newAngle = targetRotation.eulerAngles - transform.rotation.eulerAngles;
            var _ang = newAngle.NormalizeAngle().y;

            if (isStrafing && !actions && isGrounded)
        ...
        {                        
            animator.CrossFade(animationClip, transition);
        }
        ...
        void TriggerQuickStopAnimation()
        {
            ...
            if (_speedTime < -5f && quickStopConditions)            
                animator.CrossFade("QuickStop", 0.1f);            
        }
