...
namespace Invector.CharacterController
{
    public abstract class vThirdPersonMotor : vCharacter
    {
        ...
        [Tooltip("Add extra speed for the locomotion movement, keep this value at 0 if you want to use only root motion speed.")]
        public float freeSprintSpeed = 1.5f;
        [Tooltip("Add extra speed for the locomotion movement, keep this value at 0 if you want to use only root motion speed.")]
        ...
        protected void ExtraFreeSpeed(float velocity)
        {
            if (velocity <= 0) return;
            var modifier = Mathf.Clamp(velocity, 0.01f, 1f);// modifier to default velocity
            var defaultVelocity = animator.applyRootMotion ? (animator.deltaPosition / Time.deltaTime) * (velocity >= 0.01f ? modifier:1f) : Vector3.zero; //default velocity (Root motion)
            var newVelocity =(transform.forward * velocity); // New Velocity based on input (Just Forward direction)
            var vel = _rigidbody.velocity;
            vel.x = (defaultVelocity + newVelocity).x;
            vel.z = (defaultVelocity + newVelocity).z;
            _rigidbody.velocity = vel;
        }

        protected void ExtraStrafeSpeed(float velocity)
        {
            if (velocity <= 0) return;
            var modifier = Mathf.Clamp(velocity, 0.1f, 1f);// modifier to default velocity
            var defaultVelocity = animator.applyRootMotion ? (animator.deltaPosition / Time.deltaTime) * (velocity >= 0.01f ? modifier : 1f) : Vector3.zero; //default velocity (Root motion)
            var newVelocity = ((transform.forward * velocity )*input.y) + ((transform.right * velocity )* input.x); // New Velocity based on input (Forward and Side direction)
            var vel = _rigidbody.velocity;
            vel.x = (defaultVelocity + newVelocity).x;
            vel.z = (defaultVelocity + newVelocity).z;
            _rigidbody.velocity = vel;
